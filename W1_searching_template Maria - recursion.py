...
def binary_search_recursive(array, x):
    ...
    while l <= r: #while left pointer is smaller or equal the right pointer => middle
        ...
        if sorted_array[mid] == x: #if the value at the middle index is equal to the searched value
            ...
            return binary_search_recursive(sorted_array[:mid-1], x) #call the function recursively with the left half of the array    
        else:
            return binary_search_recursive(sorted_array[mid+1:], x) #call the function recursively with the right half of the array
...
class Graph:
    ...
    def graph():
        ...
        for i in range(1000, 10000, 500): #Loop from 1000 to 10000, with the step of 500
            ...
            time_binary = Graph._exec_time(binary_search_recursive, random_array, random_number)#compute exec time for binary search
